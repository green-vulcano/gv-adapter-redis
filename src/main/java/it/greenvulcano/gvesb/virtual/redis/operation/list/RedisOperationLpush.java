 ...
 *******************************************************************************/
package it.greenvulcano.gvesb.virtual.redis.operation.list;
...
public class RedisOperationLpush extends RedisOperation {
    ...
    public static final String TYPE = "lpush";

    public RedisOperationLpush() {
        super(TYPE);
    }

    @Override
    public void perform(Jedis redisConnection, String key, GVBuffer gvBuffer) throws GVException {
        gvBuffer.setProperty("REDIS_LPUSH_REPLY", redisConnection.lpush(key, gvBuffer.getObject().toString()).toString());
        
       
    	String result = redisConnection.lpush(key, gvBuffer.getObject().toString()).toString();
    ...
    public static class Builder implements RedisOperation.RedisOperationBuilder {
        ...
        public RedisOperation build(Node operationNode) throws XMLConfigException {            
            return new RedisOperationLpush();
        }
